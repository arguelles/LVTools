...
fitResult llh(LLHWorkspace& ws, std::array<double, 3>& osc_params) {
      ...
      for(unsigned int ci = 0; ci < cosZenithBins; ci++){
        for(unsigned int pi = 0; pi < energyProxyBins; pi++){
          ...
          mc_events.push_back(Event((ws.edges[year][flavor][proxy_energy_index][pi]+ws.edges[year][flavor][proxy_energy_index][pi+1])/2., // energy proxy bin center
                                    ...
                                    ws.pion_event_expectation[year][ci][pi],
                                    ws.prompt_event_expectation[year][ci][pi],
    ...
    */
    ...
    //std::vector<unsigned int> fixedIndices = {0,1,2};
...
int main(int argc, char** argv)
{
    ...
    osc_params[0] =  1.0e-24;
    osc_params[1] =  0.;
    osc_params[2] =  0.;
    auto fr = llh(llh_ws,osc_params);
    ...
    //============================= likelihood problem ends =============================//
    {
      ...
      for(Year year : {y2010,y2011}){
        for(unsigned int ci = 0; ci < cosZenithBins; ci++){
          for(unsigned int pi = 0; pi < energyProxyBins; pi++){
            // we are going to save the bin content and label it at the bin center
            mc_events.push_back(Event((llh_ws.edges[year][flavor][proxy_energy_index][pi]+llh_ws.edges[year][flavor][proxy_energy_index][pi+1])/2., // energy proxy bin center
                                      (llh_ws.edges[year][flavor][coszenith_index][ci]+llh_ws.edges[year][flavor][coszenith_index][ci+1])/2., // costh bin center
                                      year == y2010 ? 2010 : 2011, // year
                                      llh_ws.kaon_event_expectation[year][ci][pi], // amount of kaon component events
                                      llh_ws.pion_event_expectation[year][ci][pi], // amount of pion component events
                                      llh_ws.prompt_event_expectation[year][ci][pi], // amount of prompt component
                                      llh_ws.astro_event_expectation[year][ci][pi])); // astro component
          }
        }
      }

      if(argc != 8) output_file_str="./expectation.dat";
