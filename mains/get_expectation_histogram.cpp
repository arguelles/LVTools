...
int main(int argc, char** argv)
{
    ...
    std::array<double,9> parameters {1.,0.,1.,0.,0.,0.,1.0e-28,1.0e-28,1.0e-28};
    auto expectation = lv_search.GetExpectationDistribution(parameters);

    for(size_t iy=0; iy < expectation.extent(0); iy++){
      for(size_t ic=0; ic < expectation.extent(1); ic++){
        for(size_t ie=0; ie < expectation.extent(2); ie++){
          std::cout << iy << " " << ic << " " << ie << " " << expectation[iy][ic][ie] << std::endl;
        }
      }
    }
