...
static int s_cmp(const char *a, const char *b, ftnlen la, ftnlen lb)
{
	return strncmp(a, b, min(la,lb));
}

static int s_copy(char *a, char *b, ftnlen la, ftnlen lb)
{
	strncpy(a, b, min(la,lb));
	if (lb < la)
		memset(a+lb, 0, (la-lb));
	return 0;
}
...
/* Computing MAX */
    ...
    if (fold - *f <= tol) {
/*                                        terminate the algorithm. */
	s_copy(task, "CONVERGENCE: REDUCTION_OF_F_<=_TOL", (
		ftnlen)60, (ftnlen)34);
	if (iback >= 10) {
	    info = -5;
	}
